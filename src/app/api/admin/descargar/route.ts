...
export async function GET() {
  try {
    // Verificación de administrador: Se hace via header (el frontend lo enviará)
    // La validación de autenticación simple se hace desde el header
    ...
    const rows = jubilados.map((j) => ({
      ...
      "Censo Completado": j.censoCompletado ? "SÍ" : "NO",
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Censo de Jubilados");
    ...
    return new NextResponse(buffer, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="censo-jubilados-unerg-${timestamp}.xlsx"`,
